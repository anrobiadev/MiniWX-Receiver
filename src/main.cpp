...
DeviceConfig config = {
  ...
  /*aprsLogin*/ "", /*aprsServer*/ "rotate.aprs2.net", /*aprsPort*/ 14580, /*measSeconds*/ 120, /*aprsLoc0*/ "", /*aprsLoc1*/ "", /*aprsLoc2*/ "", /*aprsTempOff0*/ "0", /*aprsTempOff1*/ "0", /*aprsTempOff2*/ "0"
};
...
void aprsPoll() {
  while (aprsClient.available()) {
    ...
    if (ch == '\n') { aprsParseLine(aprsRxBuf); aprsRxBuf = ""; }
    else if (ch != '\r') {
...
void setup() {
  ...
  server.on("/", HTTP_GET, []() {
    ...
    page += "<style>body{color:#FFB000;background:#000;font-family:Arial;margin:16px;}";
    page += "label{font-size:13px;color:#9AA4B2;}";
    page += "input{font-size:16px;padding:6px;margin:4px 0 10px;width:100%;box-sizing:border-box;background:#111;color:#FFB000;border:1px solid #2A2E37;border-radius:4px;}";
    page += "select{font-size:16px;padding:6px;margin:4px 0 10px;width:100%;box-sizing:border-box;background:#111;color:#FFB000;border:1px solid #2A2E37;border-radius:4px;}";
    page += "button{font-size:16px;padding:10px;margin:6px 0;width:100%;background:#111;color:#FFB000;border:1px solid #FFB000;border-radius:6px;}";
    page += "#rb{color:#FF7A5A;border-color:#FF7A5A;}";
    page += "input[type=checkbox]{width:auto;margin-right:8px;vertical-align:middle;}";
    page += "#status{min-height:22px;font-size:14px;margin-top:10px;}</style></head><body>";
    ...
    if (LittleFS.begin()) {
      ...
      if (f) {
        String j = "{\"ssid\":\"" + config.ssid + "\",\"pass\":\"" + config.pass +
                   ...
                   "\",\"aprsPort\":\"" + intToString(config.aprsPort) + "\",\"measSeconds\":\"" + intToString(config.measSeconds) + "\",\"aprsLoc0\":\"" + config.aprsLoc0 + "\",\"aprsLoc1\":\"" + config.aprsLoc1 + "\",\"aprsLoc2\":\"" + config.aprsLoc2 + "\",\"aprsTempOff0\":\"" + config.aprsTempOff0 + "\",\"aprsTempOff1\":\"" + config.aprsTempOff1 + "\",\"aprsTempOff2\":\"" + config.aprsTempOff2 + "\"}";
        f.print(j); f.close();
      }
    }
